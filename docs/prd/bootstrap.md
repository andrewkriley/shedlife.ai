...
- Does not collect the LLM API key, the Proxmox root password, or tenant
  facts. Those belong to the web app. It *does* generate the first
  operator email + password, print them next to the URL, and seed that
  identity in the CT so the operator can log in with what the installer
  showed.
...
- Operator email + password: generated by the installer and printed with
  the URL. If the CT has no identity yet (dev / no installer seed), the
  gate still collects email + password typed twice. This *is* the first
  user — not a later Fleet apply.
