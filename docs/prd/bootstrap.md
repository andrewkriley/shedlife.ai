...
- Does not collect the LLM API key, the Proxmox *host* root password, or
  ...
  operator email + password and a CT `root` password, print them next to
  the URL (including the early "The Shed is at" line, before `/health`
  returns), set the CT password so the Proxmox console can log in, and
  seed the operator identity so the web UI accepts the printed user.
...
- API key, validated with a live, cheap provider call. Failure stays on this
  screen.
- Operator email + password: generated by the installer and printed with
  the URL. CT console login is `root` plus a separately generated
  password, also printed there. If the CT has no identity yet (dev / no
  installer seed), the gate still collects email + password typed twice.
